...
Connect conn
If Len(Request.Form("finalFilter"))=0 Then
	sql="SELECT * FROM ViewTutorInfo ORDER BY TEACHER_NAME"
Else
	sql="SELECT * FROM ViewTutorInfo WHERE "&Request.Form("finalFilter")&" ORDER BY TEACHER_NAME"
End If
...
<table width="95%" cellpadding="2" cellspacing="1" bgcolor="dimgray">
  ...
  <tr class=<% If rs("VALID") THEN %>"valid"<% ELSE %>"invalid"<% END IF %>>
    ...
    <td align=center><%=HtmlEncode(rs("RECRUIT_TYPE"))%></td>
