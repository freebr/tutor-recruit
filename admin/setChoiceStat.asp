...
Case 3	' 确认填报记录
	...
	For i=0 To UBound(arrSelTurn)
		...
		' 更新学生表信息
		sql=sql&"UPDATE STUDENT_INFO SET TUTOR_ID=@tutor_id,TUTOR_RECRUIT_ID=@rec_id,TUTOR_RECRUIT_STATUS=3,WRITEPRIVILEGETAGSTRING=dbo.addPrivilege(WRITEPRIVILEGETAGSTRING,'SATEST',''),READPRIVILEGETAGSTRING=dbo.addPrivilege(READPRIVILEGETAGSTRING,'SATEST','')"&_
						" WHERE STU_ID="&stu_id&";"
	Next
...
Case 0	' 删除记录
	For i=0 To UBound(arrSelStu)
		...
		' 更新学生表信息
		sql=sql&"UPDATE STUDENT_INFO SET TUTOR_ID=0,TUTOR_RECRUIT_ID=0,TUTOR_RECRUIT_STATUS=0,WRITEPRIVILEGETAGSTRING=dbo.removePrivilege(WRITEPRIVILEGETAGSTRING,'SATEST'),READPRIVILEGETAGSTRING=dbo.removePrivilege(READPRIVILEGETAGSTRING,'SATEST')"&_
						" WHERE STU_ID="&stu_id&";"
	Next
	For i=0 To UBound(arrSelTurn)
		arr=Split(arrSelTurn(i),"|")
...
Case 1,2	' 未提交/未确认
	For i=0 To UBound(arrSelStu)
		...
		' 更新学生表信息
		sql=sql&"UPDATE STUDENT_INFO SET TUTOR_ID=0,TUTOR_RECRUIT_ID=0,TUTOR_RECRUIT_STATUS="&stat&",WRITEPRIVILEGETAGSTRING=dbo.removePrivilege(WRITEPRIVILEGETAGSTRING,'SATEST'),READPRIVILEGETAGSTRING=dbo.removePrivilege(READPRIVILEGETAGSTRING,'SATEST')"&_
						" WHERE STU_ID="&stu_id&";"
	Next
	For i=0 To UBound(arrSelTurn)
		arr=Split(arrSelTurn(i),"|")
...
Case 4	' 已退回
	For i=0 To UBound(arrSelTurn)
		...
		sql=sql&"IF NOT EXISTS(SELECT 1 FROM TUTOR_STUDENT_APPLY_INFO WHERE APPLY_STATUS=3 AND STU_ID="&stu_id&") "&_
						"UPDATE STUDENT_INFO SET TUTOR_ID=0,TUTOR_RECRUIT_ID=0,TUTOR_RECRUIT_STATUS=4,WRITEPRIVILEGETAGSTRING=dbo.removePrivilege(WRITEPRIVILEGETAGSTRING,'SATEST'),READPRIVILEGETAGSTRING=dbo.removePrivilege(READPRIVILEGETAGSTRING,'SATEST') WHERE STU_ID="&stu_id&";"
	Next
	If Len(sql) Then conn.Execute sql
End Select
