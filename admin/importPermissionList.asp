...
Case 2	' 数据读取，导入到数据库
	...
	Function addData()
		...
		Loop
		If numNewRec>0 Then
			sql="UPDATE STUDENT_INFO SET WRITEPRIVILEGETAGSTRING=CASE WRITEPRIVILEGETAGSTRING WHEN '0' THEN 'SA6,' ELSE WRITEPRIVILEGETAGSTRING+'SA6,' END,"&_
								"READPRIVILEGETAGSTRING=CASE READPRIVILEGETAGSTRING WHEN '0' THEN 'SA6,' ELSE READPRIVILEGETAGSTRING+'SA6,' END WHERE "&_
								"CHARINDEX('SA6,',WRITEPRIVILEGETAGSTRING)=0 AND CHARINDEX('SA6,',READPRIVILEGETAGSTRING)=0 AND CHARINDEX('SA7,',WRITEPRIVILEGETAGSTRING)=0 AND CHARINDEX('SA7,',READPRIVILEGETAGSTRING)=0 AND STU_NO IN ("&sql&");"
			conn.Execute sql
		End If
		CloseConn conn
		addData=numNewRec
