...
Function getClientInfo(cli)
	...
	Connect conn
	sql="SELECT TUT_CLIENT_STATUS,TUT_STARTDATE,TUT_ENDDATE FROM TUTOR_SYSTEM_SETTINGS WHERE USE_YEAR="&curyear&" AND USE_SEMESTER="&cur_semester&" AND VALID=1"
	GetRecordSetNoLock conn,rs,sql,result
	If rs.EOF Then
...
If tutclient.SystemStatus=SYS_STATUS_CLOSED Then
...
startdate=tutclient.getOpentime(opr,SYS_OPENTIME_START)
enddate=tutclient.getOpentime(opr,SYS_OPENTIME_END)
%>