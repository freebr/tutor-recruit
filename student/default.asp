...
<script type="text/javascript" src="../scripts/student.js?t=1"></script>
</head>
...
If Not bOpen Then disabled_flag=" disabled"
sql="SELECT TURN_NUM FROM TUTOR_SYSTEM_SETTINGS WHERE USE_YEAR="&sem_info(0)&" AND USE_SEMESTER="&sem_info(1)
GetRecordSetNoLock conn,rs,sql,result
nTurn=rs("TURN_NUM")
bRefill=False

sql="SELECT * FROM VIEW_TUTOR_STUDENT_APPLY_INFO WHERE STU_ID="&Session("Stuid")
GetRecordSetNoLock conn,rs,sql,result
If Not rs.EOF Then
	For i=1 To 3
