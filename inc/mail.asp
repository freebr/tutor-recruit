...
Function sendAnnouncementEmail(mail_id,mailrcpt,arr_fieldval)
	...
	' 应用变量值到邮件标题及内容中
	For i=0 To UBound(arr_fieldval)
...
'		Set jmail = Nothing
		...
		Dim max_attempt_num:max_attempt_num=3
		Do
...
Function getThesisReviewSystemMailIdByType(d)
	Dim sem_info,curyear,cur_semester,mail_id(10),i
	Dim conn,rs,sql
	sem_info=getCurrentSemester()
	curyear=sem_info(0)
	cur_semester=sem_info(1)
	Connect conn
	sql="SELECT * FROM THESIS_REVIEW_SYSTEM WHERE USE_YEAR="&curyear&" AND USE_SEMESTER="&cur_semester
	Set rs=conn.Execute(sql)
	If Not rs.EOF Then
		For i=1 To UBound(mail_id)
			mail_id(i)=rs("MAIL_"&i)
